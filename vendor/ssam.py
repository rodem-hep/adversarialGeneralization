...
# Has been modified for taylored use!
...
class SAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, rho, **kwargs) -> None:
...
class SSAMF(SAM):
    def __init__(self, params, base_optimizer, rho, sparsity, num_samples, update_freq, **kwargs) -> None:
        assert isinstance(base_optimizer, torch.optim.Optimizer), f"base_optimizer must be an `Optimizer`"
        self.base_optimizer = base_optimizer
        ...
        super(SSAMF, self).__init__(params, base_optimizer, rho, **kwargs)
    ...
    def update_mask(self, model, train_data, **kwargs):
        ...
        with torch.enable_grad():
            for idx, sample in enumerate(train_dataloader):
                ...
                model.eval()
                outputs = model.parent.forward(edges, nodes, high, adjmat, mask)
    ...
    @torch.no_grad()
    def step(self, closure=None, pl_model=None, epoch=None, batch_idx=None, train_data=None, logger=None, **kwargs):
        ...
        if (epoch % self.update_freq == 0) and (batch_idx == 0):
            ...
            self.update_mask(pl_model, train_data)
            logger.info('Mask Lived Weight: {:.4f}'.format(self.mask_info()))
...
class SSAMD(SAM):
    def __init__(self, params, base_optimizer, 
        rho, sparsity, drop_rate, drop_strategy, growth_strategy, update_freq, T_start, T_end, **kwargs) -> None:
        assert isinstance(base_optimizer, torch.optim.Optimizer), f"base_optimizer must be an `Optimizer`"
        self.base_optimizer = base_optimizer
        ...
        self.T_end = T_end
        super(SSAMD, self).__init__(params, base_optimizer, rho, **kwargs)
