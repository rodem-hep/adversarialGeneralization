...
from franckstools.franckstools.loss_landscapes import loss_vs_weight_variation
...
def main(cfg: DictConfig) -> None:
    ...
    for key, dataset in cfg.datamodule.data_conf.datasets.items():
        ...
        loss_tensor, distances = loss_vs_weight_variation(
            model,
            trainer,
            dataloader,
            cfg.weight_range,
    ...
    torch.save(
        avg_loss_tensor,
        f"{cfg.output_dir}/{cfg.evaluated_on_dataset_type}_avgLossIncrease.pt",
    )
    torch.save(
        distances, f"{cfg.output_dir}/{cfg.evaluated_on_dataset_type}_distances.pt"
    )
