            ...
            for key, dataset in data_conf.datasets.items():
                ...
                if cfg.adversarial_export:
                    ...
                    trainer = hydra.utils.instantiate(
                        orig_cfg.trainer, inference_mode=False
                    )
                else:
                ...
                score_dict = {
                    score: T.vstack([o[score] for o in outputs]) for score in scores
                }

                log.info("Saving outputs")

                if cfg.SWA_export:
                    orig_cfg.paths.full_path = orig_cfg.paths.full_path.replace(
                        "SWAG", "SWA"
                    )
                ...
                if cfg.adversarial_export:
                    with h5py.File(
                        output_dir
                        / f"{data_conf.dataset_type}_{key2name[key]}_{dataset_nature}_adversarial.h5",
                        mode="w",
                    ) as file:
                        for score in scores:
                ...
                #             file.create_dataset(score, data=to_np(score_dict[score]))
                    ...
                    with h5py.File(
                        output_dir
                        / f"{data_conf.dataset_type}_{key2name[key]}_{dataset_nature}.h5",
                        mode="w",
                    ) as file:
                        for score in scores:
                            file.create_dataset(score, data=to_np(score_dict[score]))
