...
class JIData(Dataset):
    ...
    def getitem_with_label(self, label, idx: int):
        """Get a single jet with a specific label."""
        # Get the index of the jet with the correct label
        idx = np.where(self.labels == label)[0][idx]
        return self[idx]

    def average_image(self, num=1000, label=0):
        """Get the average of the first num jets with a specific label."""
        mean = np.zeros(self[0][0].shape)
        labels_to_take = np.where(self.labels == label)[0]
        for i in range(num):
            mean += self[labels_to_take[i]][0]
        return mean / num, num

    def plot(self) -> None:
        """Plot the average, on linear and log scales of the first 1000 jets
        for each label and also several examples for each label."""
        if self.do_plots:
            for label in range(self.n_classes):
                aver, num = self.average_image(label=label)
                aver = aver[0]
                plt.figure()
                plt.title(f"Average {label} of {num} jets")
                plt.imshow(aver, cmap="turbo")
                plt.colorbar()
                plt.savefig(f"average{label}.pdf")

                plt.figure()
                plt.title(f"Average {label} of {num} jets")
                plt.imshow(np.log(aver), cmap="turbo")
                plt.colorbar()
                plt.savefig(f"log_average{label}.pdf")

                plt.figure(figsize=(20, 4))
                plt.title(f"Examples {label}")
                for i in range(5):
                    plt.subplot(1, 5, i + 1)
                    plt.imshow(self.getitem_with_label(label, i)[0][0], cmap="turbo")
                    plt.colorbar()
                plt.savefig(f"examples{label}.pdf")

    def __getitem__(self, idx: int) -> tuple:
        """Retrives a jet from the dataset and returns it as a jetimage along
        ...
        # Return the jet image and the label
        return image, label
