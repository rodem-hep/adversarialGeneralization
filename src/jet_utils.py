...
from mattstools.mattstools.numpy_utils import (
    empty_0dim_like,
    log_clip,
    log_squash,
    undo_log_squash,
...
def build_jet_edges(
    ...
    # TODO: I think we should keep self connection for transformers to work!
...
def graph_coordinates(
    ...
    """ Convert the standard ATLAS 4 vector to a specific set of coordinates
    ...
    jet_energy = raw_high[..., 3:4]
    jet_oth = raw_high[..., 4:] if raw_high.shape[-1] > 4 else empty_0dim_like(jet_energy)

    cst_pt = raw_nodes[..., 0:1]
    cst_relpt = raw_nodes[..., 1:2]
    cst_eta = raw_nodes[..., 2:3]
    cst_phi = raw_nodes[..., 3:4]
    cst_dr = raw_nodes[..., 4:5]
    cst_e = raw_nodes[..., 5:6]
    cst_rele = raw_nodes[..., 6:7]
    cst_charge = raw_nodes[..., 7:8]
    cst_pdgid = raw_nodes[..., 8:9]
    cst_d0 = raw_nodes[..., 9:10]
    cst_dz = raw_nodes[..., 10:11]
    cst_oth = raw_nodes[..., 11:] if raw_nodes.shape[-1] > 11 else empty_0dim_like(cst_dz)

    def calculate_mass(jet_energy, jet_pt, jet_eta):
        value = jet_energy**2 - (jet_pt*np.cosh(jet_eta))**2
        if value < 0:
    ...
    high = {
        ...
        "eta": lambda: jet_eta,
        "phi": lambda: jet_phi,
        "energy": lambda: jet_energy,
        "mass": lambda: calculate_mass(jet_energy, jet_pt, jet_eta),
        ...
        "log_energy": lambda: log_clip(jet_energy),
    }

    # Put all of the constituent tensors into a single dictionary
    nodes = {
        "pt": lambda: cst_pt,
        "relpt": lambda: cst_relpt,
        "eta": lambda: cst_eta,
        "phi": lambda: cst_phi,
        "dr": lambda: cst_dr,
        "energy": lambda: cst_e,
        "relenergy": lambda: cst_rele,
        "charge": lambda: cst_charge,
        "pdgid": lambda: cst_pdgid,
        "d0": lambda: cst_d0,
        "dz": lambda: cst_dz,
        ...
        "log_energy": lambda: log_clip(cst_e),
        "pt_frac": lambda: cst_pt / (np.expand_dims(jet_pt, -1) + EPS),
        "log_squash_pt": lambda: log_squash(cst_pt),
        "tanhd0": lambda: np.tanh(cst_d0),
        "tanhdz": lambda: np.tanh(cst_dz),

        "isAntiParticle": lambda: cst_pdgid < 0,
        "isPhoton": lambda: np.abs(cst_pdgid) == 22,
        "isElectron": lambda: np.abs(cst_pdgid) == 11,
        "isMuon": lambda: np.abs(cst_pdgid) == 13,
        "isChargedHadron": lambda: np.isin(np.abs(cst_pdgid), [211, 321, 2212, 3112, 3222, 3312, 3334]),
        "isNeutralHadron": lambda: np.isin(np.abs(cst_pdgid), [0, 2112]),
    }   

    # Other derived variables require the dicts to already be created
    high["log_mass"] = lambda: log_clip(high["mass"]())
    nodes["log_energy_frac"] = lambda: nodes["log_energy"]() - np.expand_dims(
        high["log_energy"](), -1
    )
...
def graph_coordinates_old(
    ...
    high = {
        ...
        "pz": lambda: jet_pt * np.sinh(jet_eta),
        "log_pt": lambda: log_clip(jet_pt),
        "log_mass": lambda: log_clip(jet_mass),
